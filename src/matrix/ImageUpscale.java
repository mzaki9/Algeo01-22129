...
public class ImageUpscale {
    ...
    public static int bicubicInterpolate(BufferedImage image, double x, double y) {
        ...
        if ((tinggi==0 && lebar==0) || (tinggi==0 && lebar==width-1) || (tinggi==height-1 && lebar==0) || (tinggi==height-1 && lebar==width-1)){
            dasar = matrixpojok(tinggi,lebar , image);
        } else if ((tinggi==0 && lebar >0) || (tinggi>0 && lebar==0) || (lebar==width-1 && tinggi >0) || (tinggi==height-1 && lebar>0)){
        ...
        for (int i = xBase - 1; i <= xBase + 2; i++) {
            for (int j = yBase - 1; j <= yBase + 2; j++) {
                if (-1<x-i && -1<y-j && x-i<1 && y-j<1) {
                    red *= (int) BicubicSpline.spline(merah, x-i, y-j);
                    green *= (int) BicubicSpline.spline(hijau, x-i, y-j);
                    blue *= (int) BicubicSpline.spline(biru, x-i, y-j);
                }}}
        Ipixel = (0xFF << 24) |
    ...
    try {
        ...
        int newWidth = 2*inputImage.getWidth();
        int newHeight = 2*inputImage.getHeight();
